        ...
        <nav
            ...
            <div className="grid h-full max-w-lg grid-cols-5 pt-5 mx-auto font-medium">
                <NavButton href="/" icon={<HomeIcon
                    className={`size-7 ${pathname === "/" ? "fill-primary" : "fill-primary/50"}`}/>}
                           active={pathname === "/"} label="Beranda"/>
                <NavButton href="/umrah" icon={<KaabaIcon
                    className={`size-7 ${pathname === "/umrah" ? "fill-primary" : "fill-primary/50"}`}/>}
                           active={pathname === "/umrah"} label="Paket Umrah"/>
                ...
                <NavButton href="/faq" icon={<FaqIcon
                    className={`size-7 ${pathname === "/faq" ? "fill-primary" : "fill-primary/50"}`}/>}
                           active={pathname === "/faq"} label="Goumrah Care"/>
                <NavButton href="/about-us" icon={<AboutIcon
                    className={`size-7 ${pathname === "/about-us" ? "fill-primary" : "fill-primary/50"}`}/>}
                           active={pathname === "/about-us"} label="Tentang Kami"/>
...
const NavButton: React.FC<NavButtonProps> = ({href, icon, active, label}) => (
    <Link href={href} prefetch={true}
          title = {label}
          className={`group inline-flex px-5 flex flex-col items-center gap-1 ${active ? "text-primary" : "text-primary/50"}`}
