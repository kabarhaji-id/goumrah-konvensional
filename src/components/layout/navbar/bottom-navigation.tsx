  ...
  return (
    <nav className="sticky bottom-0 z-50 bg-white flex items-center justify-center gap-2 rounded-t-2xl py-4 border-t border-gray-200">
      <div className="grid h-full max-w-lg grid-cols-5 pt-5 mx-auto font-medium">
        <NavButton href="/" icon={<HomeIcon className={`size-7 ${pathname === "/" ? "fill-primary" : "fill-primary/50"}`} />} active={pathname === "/"} label="Beranda" />
        <NavButton href="/umrah" icon={<KaabaIcon className={`size-7 ${pathname === "/umrah" ? "fill-primary" : "fill-primary/50"}`} />} active={pathname === "/umrah"} label="Paket Umrah" />
        <div className="invisible group inline-flex px-5 flex flex-col items-center gap-1 text-primary/50">
          ...
          <p className="whitespace-nowrap text-xs font-light tracking-normal invisible">Tanya Isma</p>
        </div>
        <NavButton href="/faq" icon={<FaqIcon className={`size-7 ${pathname === "/faq" ? "fill-primary" : "fill-primary/50"}`} />} active={pathname === "/faq"} label="Goumrah Care" />
        <NavButton href="/about-us" icon={<AboutIcon className={`size-7 ${pathname === "/about-us" ? "fill-primary" : "fill-primary/50"}`} />} active={pathname === "/about-us"} label="Tentang Kami" />
...
const NavButton: React.FC<NavButtonProps> = ({ href, icon, active, label }) => (
  <Link href={href} prefetch={true} className={`group inline-flex px-5 flex flex-col items-center gap-1 ${active ? "text-primary" : "text-primary/50"}`} aria-current={active ? "page" : undefined}>
    {icon}
